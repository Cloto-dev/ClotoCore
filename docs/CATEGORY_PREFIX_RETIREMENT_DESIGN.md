...
**Status:** Proposed
**Scope:** CScheduler Scope #12 · Goal #143
**Author:** kernel team · 2026-07-04
**Predecessor / template:** `docs/MIND_PREFIX_RETIREMENT_DESIGN.md` (Goal #142, shipped
...
| Reasoning | `mind.` (ids already bare) | `think \| think_with_tools` | ✓ (Goal #142) |
...
3. **Residue sweep (gap left by Goal #142):** `cron_jobs.engine_id` stores engine ids but
...
## 8. Multi-repo Phase 2 — clotohub-servers (Task #155)
...
3. Classification is tool-surface only: `classify_tool` and `serverCategory.ts` keep **no**
   prefix arm. (Exception unchanged from Goal #142: `is_engine_server`'s `mind.` shortcut
