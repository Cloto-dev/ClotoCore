...
| Platform | Installer Format | 自動更新 | 優先度 |
|----------|-----------------|---------|--------|
| Windows x64 | NSIS (.exe) via Tauri | Ed25519署名 | Phase 1 |
| macOS x64 | .dmg via Tauri | Ed25519署名 | Phase 1 |
| macOS arm64 | .dmg via Tauri | Ed25519署名 | Phase 1 |
| Linux x64 | .AppImage via Tauri | Ed25519署名 | Phase 1 |
| Linux arm64 | .AppImage via Tauri | Ed25519署名 | Phase 2 |
...
開発者 (タグ push: v0.5.3)
  ...
  │     ├── cargo tauri build (マルチプラットフォーム)
  │     ├── Ed25519 署名
  │     ├── latest.json 生成
  │     └── GitHub Releases へアップロード
  │
  └── GitHub Releases (配布ポイント)
        ├── cloto-setup-0.5.3.exe         (Windows NSIS)
        ├── cloto-setup-0.5.3.exe.sig     (署名)
        ├── ClotoCore_0.5.3_amd64.AppImage (Linux)
        ├── ClotoCore_0.5.3.dmg           (macOS)
...
build-tauri:
  ...
  needs: build-dashboard  # dashboard dist を先にビルド
  strategy:
    fail-fast: false
    matrix:
      include:
...
```json
{
  ...
  "platforms": {
    "windows-x86_64": {
      ...
      "url": "https://github.com/Cloto-dev/ClotoCore/releases/download/v0.5.3/cloto-setup-0.5.3.exe.zip"
    },
    "linux-x86_64": {
      "signature": "<Ed25519 signature>",
      "url": "https://github.com/Cloto-dev/ClotoCore/releases/download/v0.5.3/ClotoCore_0.5.3_amd64.AppImage.tar.gz"
    },
    "darwin-x86_64": {
      "signature": "<Ed25519 signature>",
      "url": "https://github.com/Cloto-dev/ClotoCore/releases/download/v0.5.3/ClotoCore.app.tar.gz"
    },
    "darwin-aarch64": {
      "signature": "<Ed25519 signature>",
      "url": "https://github.com/Cloto-dev/ClotoCore/releases/download/v0.5.3/ClotoCore.app.tar.gz"
...
`scripts/generate_update_manifest.py` で `.sig` ファイルから自動生成する。
...
**移行方針**:
- Phase 1: Tauri NSIS でデスクトップアプリのインストーラーを生成
- 既存 Inno Setup (`installer/`) は CLI 専用として維持（サーバー用途）
- デスクトップユーザーには Tauri NSIS インストーラーを案内
...
2. `tauri.conf.json` に公開鍵追加、NSIS設定カスタマイズ
3. `release.yml` に `build-tauri` ジョブ追加
4. `scripts/generate_update_manifest.py` 作成
5. 既存 `build-installer` (Inno Setup) との共存確認
...
| ファイル | 説明 |
|---------|------|
| `.github/workflows/release.yml` | リリースビルド CI/CD |
| `dashboard/src-tauri/tauri.conf.json` | Tauri 設定 (NSIS, updater) |
| `installer/cloto-setup.iss` | Inno Setup 設定 (CLI用、維持) |
| `installer/build-installer.ps1` | Inno Setup ビルドスクリプト |
| `scripts/generate_update_manifest.py` | 更新マニフェスト生成 (新規) |
