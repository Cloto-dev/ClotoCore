...
- **Consensus revived** as in-kernel orchestration (Goal #138): answers are
  ...
  (#230–#232, #237, #242, #266)
- **Safe shutdown** (Goal #164): tray quit and the new sidebar power button
...
- **Per-agent recall configuration** (Goal #120): recall timing policy,
...
- **Per-server MCP log streaming** in the dashboard Log tab (Goal #141). (#244)
- **LLM provider metadata** seeded from the marketplace catalog; the agent
  ...
  (#249–#251)
- **Release pipeline** (Goal #165): Release Lifecycle Standard adoption
...
**Consensus (revival hardening, Goal #138):**
