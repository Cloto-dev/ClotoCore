...
| **Access control default** | Server-level `default_policy` fallback: `opt-out` (allow) → `opt-in` (deny), and retire the per-server policy column in favor of a single kernel-wide default. | The current `opt-out` fallback in `db::mcp::resolve_tool_access` (step 3) means any agent with no explicit `server_grant`/`tool_grant` is still permitted to call every tool on an `opt-out` server. This violates least-privilege and undermines the `mcp_access_control` table as a real boundary — explicit grants should be required, not informational. New registrations already default to `opt-in` after the 2026-04-24 fix, but legacy rows carrying `opt-out` from the 2026-03-01 migration still allow through the fallback. | Breaking for existing installs: every tool an agent previously used via fallback-allow must get an explicit `server_grant` before the flip, or the agent loses access on upgrade. A migration must either (a) synthesize `server_grant` rows for every currently-usable (agent, server) pair derived from recent call history, or (b) prompt the user through SetupWizard to re-confirm grants per agent. Plan: land after the dashboard surfaces per-tool grant editing clearly enough that Tier 1 users can recover from an unintended deny. |
...
| Interactive Actions panel (External tab buttons) | The `mgp.callback.request` notification already accepts an `options: Option<Vec<String>>` field (`crates/shared/src/lib.rs:545`) and the kernel retains it on the `PendingCallback` built at `crates/core/src/managers/mcp_events.rs:472-532`. The downstream `ExternalAction` event that `ActionsContext` consumes (`crates/shared/src/lib.rs:564-583`) has no corresponding field, so the information is dropped in the translation at `crates/core/src/events.rs:560-572`. The External tab can therefore only show prompts and free-text responses — there is no path for a bridge to surface clickable choices (Approve / Reject / Edit / Vote / …) back to the human operator. Plumbing the field end-to-end unlocks interactive external actions as a general capability. Steps: (a) add `options: Option<Vec<ActionOption>>` (`{ id, label, variant: "affirmative" \| "destructive" \| "neutral" \| "edit" }`) to `ExternalAction` and propagate at `events.rs:560-572`; (b) mirror the type into `dashboard/src/types.ts`, extend `dashboard/src/components/ExternalActionCard.tsx` to render a button row — `CommandApprovalCard.tsx` provides a ready-to-copy three-variant styling pattern; (c) add `POST /api/actions/:action_id/respond` that translates a click into an `mgp/callback/respond` via the existing `managers/mcp_events.rs:536+` path — no new MGP surface, only a UI-facing entry into an established flow; (d) optionally let the `edit` variant open an inline textarea whose contents become the response body so bridges can offer light proofreading without a bespoke UI. The capability generalizes: any external-IO bridge where the operator routinely picks from a small fixed set (e.g. approve/deny review drafts, accept/decline calendar invites, merge/close/comment on issue notifications) shares the same surface. | When a second external bridge beyond Discord text messages ships where the operator routinely needs a fixed-choice decision, and the chat-driven "ask the agent to call the right tool" loop becomes the bottleneck. |
