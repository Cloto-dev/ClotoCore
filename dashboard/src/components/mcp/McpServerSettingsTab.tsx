...
export function McpServerSettingsTab({ server, onRefresh }: Props) {
  ...
  return (
    <div className="p-4 space-y-4">
      ...
      <section>
        <h3 className="text-xs font-mono text-content-tertiary mb-2">{t('settings_tab.server_config')}</h3>
        <div className="space-y-2">
          <div className="flex items-center gap-2">
            <label className="text-xs font-mono text-content-tertiary w-20">{t('settings_tab.command')}</label>
            <span className="text-xs font-mono text-content-secondary bg-surface-panel rounded px-2 py-1 flex-1">
              {settings?.command ?? server.command ?? '—'}
            </span>
          </div>
          <div className="flex items-center gap-2">
            <label className="text-xs font-mono text-content-tertiary w-20">{t('settings_tab.args')}</label>
            <span className="text-xs font-mono text-content-secondary bg-surface-panel rounded px-2 py-1 flex-1 truncate">
              {(settings?.args ?? server.args ?? []).join(' ') || t('settings_tab.none')}
            </span>
          </div>
          <div className="flex items-center gap-2">
            <label className="text-xs font-mono text-content-tertiary w-20">{t('settings_tab.transport')}</label>
      ...
      <section>
        <h3 className="text-xs font-mono text-content-tertiary mb-2">{t('settings_tab.env_vars')}</h3>
        <EnvVariableEditor
          entries={envEntries}
      ...
      {/* Default Policy */}
      <section>
        <h3 className="text-xs font-mono text-content-tertiary mb-2">{t('settings_tab.default_policy')}</h3>
        <select
          value={defaultPolicy}
        ...
        <p className="mt-1 text-xs font-mono text-content-tertiary">
      ...
      {/* Manifest */}
      <section>
        <h3 className="text-xs font-mono text-content-tertiary mb-2">{t('settings_tab.manifest')}</h3>
      ...
      <div className="flex gap-2 pt-2 border-t border-edge">
        <button
          ...
          className="flex items-center gap-1 px-3 py-1.5 text-xs font-mono rounded bg-agent/10 hover:bg-agent/20 text-agent disabled:opacity-40 disabled:cursor-not-allowed transition-colors border border-agent/20"
        >
        ...
        <button
          ...
          className="flex items-center gap-1 px-3 py-1.5 text-xs font-mono rounded bg-surface-panel hover:bg-surface-field text-content-tertiary transition-colors border border-edge"
        >
