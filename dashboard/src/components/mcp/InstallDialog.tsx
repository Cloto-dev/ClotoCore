      ...
      <div className="px-5 py-4 space-y-4">
        ...
        <div className="space-y-1">
          <div className="flex items-center gap-2 text-xs font-mono text-content-tertiary">
            ...
            <span className="px-1.5 rounded bg-surface-secondary">{entry.category}</span>
            <span className="px-1.5 rounded bg-surface-secondary">{entry.trust_level}</span>
          </div>
          <p className="text-xs font-mono text-content-secondary leading-relaxed">{entry.description}</p>
        </div>
        ...
        {entry.runtime === 'rust' && !installing && !complete && (
          <div className="flex items-center gap-2 bg-orange-500/5 border border-orange-500/20 rounded px-3 py-2">
            ...
            <span className="text-xs font-mono text-orange-400">{t('marketplace.rust_notice')}</span>
          </div>
        )}
        ...
        {allEnvVars.length > 0 && !installing && !complete && (
          <div className="space-y-2">
            <span className="text-xs font-mono text-content-tertiary">Environment Variables</span>
            {allEnvVars.map((ev) => {
              const isRequired = entry.env_vars.some((v) => v.name === ev.name);
              return (
                <div key={ev.name}>
                  <label className="flex items-center gap-1 text-xs font-mono text-content-tertiary mb-0.5">
                    {ev.name}
                    {isRequired && <span className="text-red-400">*</span>}
                  </label>
                  {ev.description && <p className="text-xs font-mono text-content-tertiary mb-0.5">{ev.description}</p>}
                  <input
                    type="text"
          ...
          <div className="space-y-2">
            {steps.map((step) => (
              <div key={step.step} className="flex items-center gap-2">
                {step.status === 'running' && <Loader size={12} className="text-agent animate-spin shrink-0" />}
                ...
                <div className="flex-1 min-w-0">
                  <span className="text-xs font-mono text-content-secondary">{step.description ?? step.step}</span>
                  {step.detail && <span className="text-xs font-mono text-content-tertiary ml-2">{step.detail}</span>}
                  {step.error && <p className="text-xs font-mono text-red-400 mt-0.5">{step.error}</p>}
                </div>
              </div>
            ))}
          </div>
        ...
        {complete && (
          <div className="flex items-center gap-2 text-emerald-500">
            <CheckCircle size={14} />
            <span className="text-xs font-mono">{t('marketplace.complete')}</span>
          </div>
        )}
        ...
        {installError && (
          <div className="flex items-center gap-2 text-red-400 bg-red-500/5 border border-red-500/20 rounded px-3 py-2">
            ...
            <span className="text-xs font-mono">{installError}</span>
          </div>
        )}
        ...
        {!complete && (
          <div className="flex justify-end gap-2 pt-1">
            <button
              ...
              className="px-3 py-1.5 text-xs font-mono rounded bg-surface-panel hover:bg-surface-field text-content-tertiary transition-colors border border-edge disabled:opacity-40"
            >
        ...
        {complete && (
          <div className="flex justify-end pt-1">
            <button
              onClick={onClose}
              aria-label="OK"
              className="px-3 py-1.5 text-xs font-mono rounded bg-agent/10 hover:bg-agent/20 text-agent transition-colors border border-agent/20"
            >
              OK
            </button>
          </div>
