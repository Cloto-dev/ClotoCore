      ...
      <div className="px-5 py-4 space-y-4">
            ...
            {allEnvVars.map((ev) => {
              ...
              return (
                <div key={ev.key}>
                  ...
                  {ev.description && (
                    <p className="text-[9px] font-mono text-content-tertiary mb-0.5">{ev.description}</p>
                  )}
                  <input
                    type="text"
        ...
        {complete && (
          <div className="flex items-center gap-2 text-emerald-500">
            <CheckCircle size={14} />
            <span className="text-[10px] font-mono">{t('marketplace.complete')}</span>
          </div>
        )}
        ...
        {installError && (
          <div className="flex items-center gap-2 text-red-400 bg-red-500/5 border border-red-500/20 rounded px-3 py-2">
            ...
            <span className="text-[10px] font-mono">{installError}</span>
          </div>
        )}
        ...
        {!complete && (
          <div className="flex justify-end gap-2 pt-1">
            <button
              onClick={onClose}
              disabled={installing}
              className="px-3 py-1.5 text-[10px] font-mono rounded bg-glass hover:bg-glass-strong text-content-tertiary transition-colors border border-edge disabled:opacity-40"
            >
        ...
        {complete && (
          <div className="flex justify-end pt-1">
            <button
              onClick={onClose}
              className="px-3 py-1.5 text-[10px] font-mono rounded bg-brand/10 hover:bg-brand/20 text-brand transition-colors border border-brand/20"
            >
              OK
            </button>
          </div>
