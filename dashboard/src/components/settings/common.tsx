  ...
  return (
    <label className="flex items-center justify-between cursor-pointer select-none group">
      ...
      <button
        ...
        aria-label={label}
        className={`w-10 h-5 rounded-full transition-colors relative ${enabled ? 'bg-agent' : 'bg-surface-secondary border border-edge'}`}
      >
        <div
...
export function SectionCard({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="bg-surface-field p-5 rounded-lg border border-edge shadow-sm mb-4">
      <h3 className="text-xs font-black text-content-tertiary mb-4">{title}</h3>
      {children}
    </div>
  );
}
