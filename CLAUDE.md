...
- Rust lint: `bash scripts/lint-rust.sh` (the CI Lint job's clippy invocation, read out of `ci.yml`)
...
against a live headless `clotocore` kernel over its HTTP admin API (CSC Goal
#169) — the operation-coverage complement to the boot-only `--smoke` and the
