...
async fn test_permission_grant_flow() {
    ...
    let status: String =
        sqlx::query_scalar("SELECT status FROM permission_requests WHERE request_id = ?")
            .bind("req-123")
...
async fn test_config_update_flow() {
    ...
    state
        .plugin_manager
        .update_config("test.plugin", "api_key", "new_value")
        .await
        .unwrap();
...
async fn test_agent_creation_with_memory_context() {
    ...
    let stored_agent: (String, String) =
        sqlx::query_as("SELECT name, description FROM agents WHERE name = ?")
